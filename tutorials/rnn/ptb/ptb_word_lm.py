...
flags.DEFINE_string("seed_for_sample", "domestic sales of",
...
class PTBModel(object):
  ...
  def __init__(self, is_training, config):
    ...
    #self.sample = tf.argmax(tf.nn.softmax(logits), 1)
...
def main(_):
  ...
  if config.is_char_model:
    seed_for_sample = [c for c in FLAGS.seed_for_sample.replace('\'','').replace(' ', '_')]
  else:
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as session:
      ...
      for v in variables:
        print(v.name)
        params[v.name]=v.eval(session=session)       
      print('END OF ALL TRAINABLE VARIABLED')
